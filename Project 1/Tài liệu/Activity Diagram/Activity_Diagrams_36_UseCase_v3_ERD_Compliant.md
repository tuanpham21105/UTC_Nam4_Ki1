...
*(🛠 sửa lại đúng theo Detailed_UseCase_Specifications.md — UC20 thực chất KHÔNG phải "Lưu và khôi phục bài làm" mà là bước rà soát bài làm trước khi chuyển sang UC21)*

```mermaid
flowchart TD
    Start((Bắt đầu))
    subgraph ACTOR["👤 Học viên"]
        a1["Đã làm xong (một phần/toàn bộ) bài Nghe/Nói/Đọc/Viết — «cross-ref» UC16-19"]
        a2[Chọn Kiểm tra bài làm]
        a3["Xem lại từng câu trả lời theo trạng thái hiển thị"]
        d0{"Cần chỉnh sửa câu nào không?"}
        a4["Quay lại làm bài (chỉnh sửa) — «cross-ref» UC16-19"]
        a5["Xác nhận bài đã sẵn sàng"]
    end
    subgraph SYS["🖥️ Hệ thống"]
        s1["Kiểm tra assignments.status = OPEN và close_at chưa qua"]
        d1{"Bài tập còn hạn (chưa CLOSED)?"}
        s1b["Chặn — bài đã hết hạn, xử lý theo chính sách (không cho kiểm tra/nộp tiếp)"]
        s2["SELECT answers JOIN submission_modules JOIN questions<br/>WHERE submission_module_id thuộc submissions hiện tại (status=IN_PROGRESS)"]
        s3["Hiển thị trạng thái từng câu: đã trả lời / chưa trả lời<br/>(so khớp questions.id với answers.question_id hiện có)"]
        d2{"Còn questions chưa có answers tương ứng?"}
        s4["Cảnh báo: còn X câu chưa trả lời"]
        d3{"content/audio_storage_key hợp lệ theo đúng question_type/skill?"}
        s5["Yêu cầu sửa lại dữ liệu không hợp lệ<br/>(vd: SHORT_ANSWER rỗng, Speaking chưa có audio_upload_status=READY)"]
        s6["Cho phép chỉnh sửa: quay lại UPSERT answers ở UC16-19"]
        s7["Đánh dấu bài làm đã sẵn sàng, chuyển sang «include» Nộp bài tập UC21<br/>(KHÔNG đổi submissions.status — vẫn IN_PROGRESS cho tới khi UC21 xác nhận)"]
    end
    End((Kết thúc))

    Start --> a1 --> a2 --> s1 --> d1
    d1 -->|Hết hạn| s1b --> End
    d1 -->|Còn hạn| s2 --> s3 --> a3 --> d2
    d2 -->|Có câu chưa trả lời| s4 --> d0
    d2 -->|Đã trả lời hết| d3
    d3 -->|Không hợp lệ| s5 --> d0
    d3 -->|Hợp lệ| d0
    d0 -->|Có, cần sửa| a4 --> s6 --> a2
    d0 -->|Không, đã ổn| a5 --> s7 --> End
```

**Cột/bảng ERD dùng:** `assignments(status, close_at)`, `submissions(status=IN_PROGRESS)`, `submission_modules`, `answers(content, audio_storage_key, audio_upload_status, question_id)`, `questions(id, question_type)`.
**Tuân thủ đặc tả UC20 (Detailed_UseCase_Specifications.md, mục 20):** Đây là UC riêng biệt nằm **giữa** UC16-19 (làm bài) và UC21 (nộp bài) — không phải cơ chế lưu/khôi phục dữ liệu. Sơ đồ bám sát đúng dòng sự kiện chính: *chọn kiểm tra → tải toàn bộ câu trả lời → hiển thị trạng thái từng câu → học viên kiểm tra → cảnh báo phần chưa hoàn thành → chỉnh sửa nếu cần → xác nhận sẵn sàng → chuyển sang UC21*, và đủ 4 dòng sự kiện phụ đã nêu: (1) có câu chưa trả lời → cảnh báo; (2) dữ liệu không hợp lệ → yêu cầu sửa; (3) học viên quay lại làm bài → cho phép chỉnh sửa; (4) bài hết hạn → chặn, không cho tiếp tục. Toàn bộ thao tác chỉ là SELECT/hiển thị trên `answers`/`questions` đã có sẵn — không cần bảng mới, không đổi `submissions.status` (vẫn `IN_PROGRESS`, chỉ chuyển hẳn sang `SUBMITTED` ở UC21 theo đúng mục 5.6 đã chốt trước đó).
